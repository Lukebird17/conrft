...
def save_demos(successes, exp_name, success_needed):
    os.makedirs(f"./classifier_data/classifier_data_{exp_name}", exist_ok=True)
    uuid = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    success_file = f"./classifier_data/classifier_data_{exp_name}/{exp_name}_{success_needed}_success_images_{uuid}.pkl"
    # failure_file = f"./classifier_data/{exp_name}_failure_images_{uuid}.pkl"

    with open(success_file, "wb") as f:
        pkl.dump(successes, f)
        print(f"✅ Saved {len(successes)} successful transitions to {success_file}")
...
def main(_):
    ...
    print(f"[Info] Loaded {len(successes)} successes and from previous session.")

    success_needed = FLAGS.successes_needed
    pbar = tqdm(total=success_needed)
    pbar.update(len(successes))
    ...
    while len(successes) < success_needed:
        ...
        if key_flags["success"]:
            successes.append(transition)
            print("✨ success plus one ✨")
            pbar.update(1)
            key_flags["success"] = False  # 重置按键状态




    save_demos(successes, FLAGS.exp_name, success_needed)
